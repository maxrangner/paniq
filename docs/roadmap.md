...
**Beside the prototype, not a stone of it: implemented and in use
(2026-09-26).** The [model pipeline](model-pipeline.md), the way a thing
described in words or sketched on paper becomes a mesh the game can draw, is
merged into `main` and has made its first model, a wet-floor sign. Asking
for a model is now part of normal work. Nothing on screen has changed yet:
the stone that draws models in place of capsules and blocks is still to
come.
...
door (the closet later grew north, on 2026-09-24, keeping its door and its old
floor). Dozens of tests name places inside them by coordinate. Moving them would
...
| Props face their walls | Style | The vending machine, the filing cabinets, the copier, the microwaves, the sockets, the fuse box and the shelves each turn their back to the wall they stand against, so drawers, glass fronts and bookshelves face the room. They all used to face north, so the ones on the east and west walls stood side-on to their wall. Their collision boxes turn with them, so the crowd bumps them differently |
| A door between the meeting room and the cafeteria | System | The meeting room's east wall has a door straight into the cafeteria, at the far end from the corridor. From the meeting table the shortest way out is now through the cafeteria and its shortcut rather than down the whole corridor to the T, so the meeting and the cafeteria empty into the same doorway |
| A longer closet | System | The storage closet runs from the office's east door north to the corridor wall: 2 m by 4.5 m instead of 2 m square, with two shelves of stores along its far wall. It keeps its door and every spot the tests name in it. As a refuge it now holds nine people instead of four |
| Fear spreads by sight | Behaviour | Anyone who sees a frightened person leap up or run, within eight metres and along a line of sight, is startled by that alone, each a few ticks late and no two on the same tick; the brave look up first. The meeting used to rise one seat at a time, seconds apart; now it comes up in a ragged wave over two or three seconds. The owner's choice |
| The first shout comes at once | Behaviour | Somebody frightened shouts with their first stride, a few ticks late like every reaction, and a shout is understood six metres off and heard twelve, halved by a shut door. It used to come two to five seconds in and be understood at two and a half metres |
| People see the room they are in | Behaviour | A threat is seen twelve metres off instead of three, and through an open doorway only when the doorway lies on the line of sight: the wall beside an open door now hides the room beyond it, where before "joined by an open door" meant seeing through the wall as readily as through the door |
| Several noises at once | Behaviour | A person keeps up to three noises in mind. Looking toward a thud in the next room, they still hear the fire crackling nearer and turn to that at once; a noise set aside is looked at next while it is still fresh; the same noise again turns no head twice. A fire is heard further as it grows, and half as far through a wall or a shut door |
| Going to see what that was | Behaviour | Somebody who hears a threat's noise from another room and sees nothing goes to look: to the door, opens it, and there it is. Not the very nervous, and nobody with a cue waiting on them. This was not on the owner's list; it is what the list needed, because on seeds 40 and 42 a bathroom ablaze was heard by eighteen people who sat on at their desks until it came through the door. They now all take fright a good half minute earlier -- and, until the way blocked by the flames is dealt with (the next stone), most still die, because the bathroom pair never get out to raise the alarm |
| A door is held for whoever is coming | Behaviour | Nobody shuts a door on somebody still coming through it -- not on their way out of a burning room, not standing beside a door with fire beyond, not wedging one shut. The one exception is the callous (compassion 3 or less) once the flames are at the door itself, which is the selfish calculated risk the owner allowed. It used to take compassion 7 to hold a door at all, so most of the office shut doors in each other's faces. The cruel still slam and lock behind them as before |
| Dash past or hide | Behaviour | When the only way out is through the heat, the brave run for it while the floor is walkable, and so do the desperate -- anyone whose own room is alight, or who has nowhere cooler to hide; the rest give the door up and hide in the nearest dead end that is not itself through the flames, and shut its door. Nobody crosses burning floor. On seed 42 the two in the bathroom used to shuttle at the edge of the heat with the door three metres off until it took them; now both dash out and shut the bathroom behind them, and with the way out opened at the same moment the seed saves twelve of twenty instead of one. The owner's choice |
| The carrier runs | Behaviour | Somebody fetching an extinguisher to the flames runs to them like the frightened person they are. They used to walk at a stroll unless somebody was alight, which is what the owner watched on seed 41 |
| Carried through the doorway | System | Somebody down inside an open doorway with the crowd pressing on them from one side is carried on through it by the press, and once out through the wall line of a way out slides on out: an escape on their back. They used to lie in the gap as a plug that nobody could pass. On seed 41 with the crowd already packed at the exit when it opens, fifteen are out within fifteen seconds; before, nobody was out in twenty-five. What the owner saw as "one agent stuck in the door" was somebody knocked out cold in the gap by the cruel shoving the weaker to the floor, which still happens -- it is the crush -- but no longer plugs the door |
| Three cards | System | The deck the dead deal from is Beefcake, TNT and the fire extinguisher. The four trait cards, "start a fire" and "pop the fuse box" are still in the code for a level that wants them; the office never deals them. The owner's call |
| Thirty and a card to start | System | A round opens with 30 influence and one card drawn from the deck, the same card on the same seed. It used to open with nothing. Thirty is one move -- a card, or a pull of the alarm -- and not the 80 the way out costs. The owner's call |
| Pull the alarm yourself | System | Every red alarm box is a thing you can click, for 30: every bell in the building rings as when a person hits one, and the level-headed walk out. Free and pointless once the bells are ringing. The story says "you pulled a fire alarm". The simulation half of the owner's ask; the click on the screen is the controls stone below |
