...
namespace Paniq.Tests.EditMode
{
    ...
    public sealed class FireReactionHardKnocksEditModeTests
    {
        ...
        public void HardHit_KnocksSomeoneOutColdThenTheyComeToAndGetUp()
        {
            ...
            {
                simulation.Step();
                FireReactionAgentSnapshot agent = simulation.GetAgent(0);
            ...
            List<CausalEvent> passedOut = EventsOfType(simulation, FireReactionEventType.AgentPassedOut);
            List<CausalEvent> cameTo = EventsOfType(simulation, FireReactionEventType.AgentCameTo);
            List<CausalEvent> gotUp = EventsOfType(simulation, FireReactionEventType.AgentGotUp);
            ...
            Assert.That(simulation.EventLog.Get(passedOut[0].CausalParentEventId).EventType,
                Is.EqualTo(FireReactionEventType.AgentTripped), "Knocked out by the fall the box caused.");
        ...
        public void PanickedCrowds_SometimesKnockSomeoneOut()
        {
            ...
            for (ulong seed = 40UL; seed <= 46UL; seed++)
            {
                ...
                foreach (CausalEvent record in EventsOfType(simulation, FireReactionEventType.AgentPassedOut))
                {
                    knockouts++;
