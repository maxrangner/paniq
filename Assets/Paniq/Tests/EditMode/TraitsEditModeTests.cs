...
namespace Paniq.Tests.EditMode
{
    ...
    public sealed class TraitsEditModeTests
    {
        ...
        private static AgentDefinition Person(ulong id, int x, int z, AgentTraitValues traits)
        {
            return new AgentDefinition(new SimulationId(id), new LogicalPosition(x, z), CardinalDirection.North, traits);
        }

        private static AgentDefinition Undecided(ulong id, int x, int z)
        {
            return new AgentDefinition(new SimulationId(id), new LogicalPosition(x, z), CardinalDirection.North);
        }
        ...
        public void DrawnTraits_AreInRangeAndReplayable()
        {
            ...
            var first = new Run(data, 7UL);
            var again = new Run(data, 7UL);
            bool differsAcrossSeeds = false;
            for (int seed = 1; seed <= 5 && !differsAcrossSeeds; seed++)
            {
                differsAcrossSeeds = !new Run(data, (ulong)seed).GetAgent(0).Traits.Equals(first.GetAgent(0).Traits);
            }
        ...
        public void SpeedTrait_SetsWalkingAndSprintingPace()
        {
            ...
            var simulation = new Run(data);
            AgentSnapshot slow = simulation.GetAgent(0);
            AgentSnapshot ordinary = simulation.GetAgent(1);
            AgentSnapshot fast = simulation.GetAgent(2);
