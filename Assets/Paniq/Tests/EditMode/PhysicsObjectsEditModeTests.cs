...
{
    public sealed class PhysicsObjectsEditModeTests
    {
        ...
        private ScenarioData PersonAndBox(int boxX, int size, int massGrams)
        {
        ...
        private static bool InAnyRoom(ScenarioData data, LogicalPosition position, int radius)
        {
        ...
        /// went through a wall.
        /// </summary>
        private static bool ThroughAnOpenWayOut(RunSnapshot snapshot, LogicalPosition position)
        {
        ...
        private static void AssertNothingOverlaps(Run simulation, ScenarioData data, string context,
            PressWatch presses, HashSet<SimulationId> wentOutside = null)
        {
            ...
            {
                PhysicsObjectSnapshot box = snapshot.PhysicsObjects[b];
        ...
        public void LowerGravity_KeepsATossedBoxInTheAirLonger()
        {
            ...
            {
                ScenarioData data = PersonAndBox(-3000, 300, 3000);
                data.Agents = new[]
                {
                    new AgentDefinition(new SimulationId(1UL), new LogicalPosition(4000, 4000), CardinalDirection.North)
                };
        ...
        {
            ScenarioData data = PersonAndBox(4000, 400, 6000);
            data.Agents = new[]
            {
                new AgentDefinition(new SimulationId(1UL), new LogicalPosition(-4000, 4000), CardinalDirection.North)
            };
        ...
        {
            ScenarioData data = PersonAndBox(-1000, 600, 20000);
            var simulation = new Run(data);
            var presses = new PressWatch();
        ...
        {
            ScenarioData data = PersonAndBox(-1000, 300, 3000);
            var simulation = new Run(data);
            var presses = new PressWatch();
            simulation.LaunchObjectForTests(0, 60, 0);
            ...
            Assert.That(EventsOfType(simulation, CausalEventType.BoxHitAgent), Is.Empty);
        ...
        public void PanickedCrowds_KickBoxesAroundWithoutOverlapping()
        {
            ...
            for (ulong seed = 40UL; seed <= 49UL; seed++)
            {
                ...
                foreach (CausalEvent bump in EventsOfType(simulation, CausalEventType.BoxBumped))
                {
                    bumps++;
        ...
        public void RunnersCanTripOverBoxes()
        {
            ...
            for (ulong seed = 40UL; seed <= 49UL && boxTrips == 0; seed++)
            {
                ...
                foreach (CausalEvent trip in EventsOfType(simulation, CausalEventType.AgentTripped))
                {
                    if (simulation.EventLog.Get(trip.CausalParentEventId).EventType == CausalEventType.BoxBumped)
                    {
                        boxTrips++;
                    }
                }
