...
namespace Paniq.Tests.EditMode
{
    ...
    public sealed class FireReactionFlammablesEditModeTests
    {
        ...
        /// </summary>
        private FireReactionScenarioData BoxBesideTheFire(params FireReactionAgentDefinition[] people)
        {
            FireReactionScenarioData data = scenario.ToRuntimeData();
            data.Agents = people.Length > 0
                ? people
                : new[] { new FireReactionAgentDefinition(new SimulationId(1UL), new LogicalPosition(5000, -5000), CardinalDirection.North) };
            data.Tables = new FireReactionTableDefinition[0];
            data.PhysicsObjects = new[]
            {
                new FireReactionPhysicsObjectDefinition(BoxId, PhysicsObjectKind.Box, new LogicalPosition(1000, 250), 400, 6000)
            };
        ...
        {
            FireReactionScenarioData data = BoxBesideTheFire();
            ...
            List<CausalEvent> caught = EventsOfType(simulation, FireReactionEventType.ObjectCaughtFire);
            ...
            List<CausalEvent> spread = EventsOfType(simulation, FireReactionEventType.FireSpread);
            ...
            List<CausalEvent> burntOut = EventsOfType(simulation, FireReactionEventType.ObjectBurntOut);
        ...
        public void PersonTouchingABurningBox_CatchesFire()
        {
            ...
            FireReactionScenarioData data = BoxBesideTheFire(new FireReactionAgentDefinition(new SimulationId(1UL),
            ...
            Assert.That(simulation.EventLog.Get(caught[0].CausalParentEventId).EventType,
                Is.EqualTo(FireReactionEventType.ObjectCaughtFire));
        }
        ...
        public void DefaultRoom_FurnitureAndBoxesBurnInMostRuns()
        {
            ...
            for (ulong seed = 40UL; seed <= 46UL; seed++)
            {
                ...
                foreach (CausalEvent record in simulation.EventLog.Events)
                {
                    if (record.EventType == FireReactionEventType.ObjectCaughtFire)
                    {
                        caught++;
                    }
                    else if (record.EventType == FireReactionEventType.FireSpread &&
                             simulation.EventLog.Get(record.CausalParentEventId).EventType == FireReactionEventType.ObjectCaughtFire)
                    {
                        squaresLitByThings++;
                    }
                }
