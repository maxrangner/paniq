...
namespace Paniq.Tests.EditMode
{
    ...
    /// </summary>
    public sealed class CrossRoomEditModeTests
    {
        ...
        private ScenarioData TwoRooms()
        {
            ...
            data.Doors = new[]
            {
                new DoorDefinition(BetweenThem, WhereTheFireIs, WallSide.East, 0, 1000, false),
        ...
        {
            ScenarioData data = TwoRooms();
            data.PhysicsObjects = new[]
            {
                new PhysicsObjectDefinition(
            ...
            bool sprayed = false;
            for (int tick = 0; tick < 60 * Run.TicksPerSecond && !sprayed; tick++)
            {
                simulation.Step();
                foreach (CausalEvent record in simulation.EventLog.Events)
                {
                    sprayed |= record.EventType == CausalEventType.ExtinguisherSprayed;
                }
            }
        ...
        public void ACalmPerson_SometimesWandersIntoTheNextRoom()
        {
            ...
            var people = new AgentDefinition[6];
            for (int i = 0; i < people.Length; i++)
            {
                people[i] = new AgentDefinition(
            ...
            for (int tick = 0; tick < 120 * Run.TicksPerSecond && !wentNextDoor; tick++)
            {
                simulation.Step();
