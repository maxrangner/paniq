...
namespace Paniq.Tests.EditMode
{
    ...
    /// </para>
    /// </summary>
    public sealed class RoundEditModeTests
    {
        ...
        /// sealed building.
        /// </summary>
        private ScenarioData LevelData()
        {
        ...
        public void TheTrigger_StartsTheFireWhereTheSeedSaysAndOnlyOnce()
        {
            using (var simulation = new Run(LevelData()))
            {
                ...
                {
                    starts += record.EventType == CausalEventType.FireActivated ? 1 : 0;
                }
        ...
        public void ARoundEnds_AndEverybodyIsAccountedFor()
        {
            ...
            using (var simulation = new Run(data))
            {
                ...
                int ticks = 0;
                const int limit = 600 * Run.TicksPerSecond;
                ...
                Assert.That(snapshot.RoundIsOver, Is.True,
                    $"The round was still going after {ticks / Run.TicksPerSecond} seconds.");
        ...
        public void SomebodyFrozenWithNothingElseHappening_EndsTheRoundAndCountsAsSaved()
        {
            ...
            ScenarioData data = SealedCloset();
            data.Agents = new[]
            {
            ...
            using (var simulation = new Run(data))
            {
                ...
                int ticks = 0;
                const int limit = 120 * Run.TicksPerSecond;
                while (simulation.Phase != RoundPhase.Over && ticks < limit)
                {
                    simulation.Step();
                    ticks++;
        ...
        public void AQueueAtADoor_NeverEndsTheRound()
        {
            ...
            ScenarioData data = SealedCloset();
            var crowd = new AgentDefinition[6];
            for (int i = 0; i < crowd.Length; i++)
            {
                crowd[i] = new AgentDefinition(new SimulationId((ulong)(1 + i)),
            ...
            using (var simulation = new Run(data))
            {
                ...
                int pressingTicks = 0;
                const int limit = 60 * Run.TicksPerSecond;
        ...
        /// </summary>
        private ScenarioData SealedCloset()
        {
            ScenarioData data = LevelData();
            ...
            data.Doors = new[]
            {
                new DoorDefinition(ClosetDoor, OfficeRoom, WallSide.East, 2500, 1000, true)
            };
        ...
        private static bool AnybodyIsMoving(Run simulation)
        {
        ...
        /// </summary>
        private static bool AnybodyIsPressing(Run simulation)
        {
            for (int i = 0; i < simulation.AgentCount; i++)
            {
