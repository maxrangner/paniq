...
namespace Paniq.Tests.EditMode
{
    ...
    /// somebody.
    /// </summary>
    public sealed class FireReactionPossessionsEditModeTests
    {
        ...
        private static FireReactionPhysicsObjectSnapshot Thing(FireReactionSimulation simulation, SimulationId id)
        {
        ...
        /// </summary>
        private FireReactionScenarioData HoldingABagInFrontOfAFire(AgentTraitValues carrier)
        {
            FireReactionScenarioData data = scenario.ToRuntimeData();
            data.Agents = new[]
            {
                new FireReactionAgentDefinition(Carrier, new LogicalPosition(0, 0), CardinalDirection.South, carrier, TheBag),
                new FireReactionAgentDefinition(Bystander, new LogicalPosition(1500, 0), CardinalDirection.North,
                    AgentTraitValues.AllOrdinary)
            };
            data.PhysicsObjects = new[]
            {
                new FireReactionPhysicsObjectDefinition(TheBag, PhysicsObjectKind.Bag, new LogicalPosition(0, 0), 350, 4000)
            };
        ...
        {
            var simulation = new FireReactionSimulation(HoldingABagInFrontOfAFire(AgentTraitValues.AllOrdinary));
            FireReactionPhysicsObjectSnapshot bag = Thing(simulation, TheBag);
        ...
        public void AFrightenedCarrier_FlingsWhatTheyWereHolding()
        {
            ...
            List<CausalEvent> thrown = EventsOfType(simulation, FireReactionEventType.ItemThrown);
        ...
        public void AVeryNervousCarrier_FumblesItOntoTheFloorInstead()
        {
            ...
            Assert.That(EventsOfType(simulation, FireReactionEventType.ItemDropped), Is.Not.Empty,
        ...
        {
            FireReactionScenarioData data = HoldingABagInFrontOfAFire(AgentTraitValues.AllOrdinary);
            data.Agents[0] = new FireReactionAgentDefinition(Carrier, new LogicalPosition(0, 0), CardinalDirection.South,
                AgentTraitValues.AllOrdinary, new SimulationId(9999UL));

            Assert.That(() => new FireReactionSimulation(data), Throws.InvalidOperationException);
        }
        ...
        {
            FireReactionScenarioData data = HoldingABagInFrontOfAFire(AgentTraitValues.AllOrdinary);
            data.PhysicsObjects[0] = new FireReactionPhysicsObjectDefinition(
