...
{
    public sealed class SimulationEditModeTests
    {
        ...
        public void DefaultScenario_IsValidWithReplayIdentity()
        {
            ...
            Assert.That(data.ContentRevision, Is.EqualTo("63"));
            Assert.That(data.SimulationCompatibilityVersion, Is.EqualTo(51));
        ...
        {
            var asset = AssetDatabase.LoadAssetAtPath<ScenarioAsset>(ScenarioAssetPath);
        ...
        public void Fire_StartsAsOneCellAtTick250()
        {
            ...
            Assert.That(simulation.EventLog.Get(snapshot.FireCells[0].EventId).EventType,
                Is.EqualTo(CausalEventType.FireActivated));
        }
        ...
        /// </summary>
        private ScenarioData NobodyFightsTheFire()
        {
            ScenarioData data = DefaultData();
        ...
        {
            var simulation = new Run(NobodyFightsTheFire());
            ...
            for (int i = 0; i < cells.Count; i++)
            {
                ...
                if (i == 0)
                {
                    Assert.That(ignition.EventType, Is.EqualTo(CausalEventType.FireActivated));
                }
                else
                {
                    Assert.That(ignition.EventType, Is.EqualTo(CausalEventType.FireSpread));
                    CausalEventType cause = simulation.EventLog.Get(ignition.CausalParentEventId).EventType;
                    if (cause == CausalEventType.ObjectCaughtFire ||
                        cause == CausalEventType.ObjectExploded)
                    {
        ...
        private static LogicalBounds RoomHolding(ScenarioData data, LogicalPosition position)
        {
        ...
        private static bool InADoorway(RunSnapshot snapshot, LogicalPosition position)
        {
        ...
        public void Fire_FillsTheRoomWithinAMinute()
        {
            ...
            for (int i = 0; i < 60 * Run.TicksPerSecond && simulation.FireCellCount < officeCells; i++)
            {
                simulation.Step();
            }
        ...
        {
            var first = new Run(DefaultData());
            var second = new Run(DefaultData());
            for (int tick = 0; tick < 3000; tick++)
            {
                ...
                {
                    AgentSnapshot a = first.GetAgent(i);
                    AgentSnapshot b = second.GetAgent(i);
        ...
        {
            var first = new Run(DefaultData(), 42UL);
            var second = new Run(DefaultData(), 43UL);
            bool differs = false;
        ...
        public void Movement_KeepsParticipatingAgentsInsideTheRoomAndSeparated()
        {
            ...
            for (int tick = 0; tick < 3000; tick++)
            {
                ...
                for (int i = 0; i < snapshot.Agents.Count; i++)
                {
                    ...
                    for (int j = 0; j < i; j++)
                    {
                        AgentSnapshot other = snapshot.Agents[j];
        ...
        public void CalmAgents_WanderOnCurvedPathsPauseAndAvoidWalls()
        {
            ...
            for (int tick = 0; tick < 60 * Run.TicksPerSecond; tick++)
            {
                simulation.Step();
                for (int i = 0; i < count; i++)
                {
        ...
        public void UprightPeople_NeverTurnFasterThanTheirTurnRate(ulong seed)
        {
            ...
            var before = new AgentSnapshot[simulation.AgentCount];
            ...
            for (int tick = 0; tick < 3000; tick++)
            {
                ...
                for (int i = 0; i < before.Length; i++)
                {
                    AgentSnapshot now = simulation.GetAgent(i);
        ...
        public void PanickedAgents_SprintZigZagAndDoNotStayPinned()
        {
            ...
            int endTick = data.Fire.ActivationTick + 20 * Run.TicksPerSecond;
            while (simulation.Tick < endTick)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    else
                    {
                        ...
                        // RoomsEditModeTests owns that case and
                        // holds it to its own, looser limit; what this guards
                        // against is somebody stuck against a wall in open floor.
                        bool queueing = RoomsEditModeTests.NearAnOpenDoor(simulation, agent.Position);
            ...
            Assert.That(scaredSamples, Is.GreaterThan(30 * Run.TicksPerSecond),
        ...
        public void Vision_SeesFireAheadButNotBehind()
        {
            ...
            ScenarioData data = DoorsEditModeTests.WithAWayOutOfTheOffice(DefaultData());
            ...
            AgentSnapshot fled = simulation.GetAgent(0);
        ...
        public void VisualAlert_ProducesYellAndPropagatesReactionWithCausalParents()
        {
            ...
            Assert.That(events[0].EventType, Is.EqualTo(CausalEventType.FireActivated));
            Assert.That(events[1].EventType, Is.EqualTo(CausalEventType.AgentAlerted));
            Assert.That(events[1].CausalParentEventId, Is.EqualTo(events[0].EventId));
            Assert.That(events[2].EventType, Is.EqualTo(CausalEventType.AgentYelled));
            Assert.That(events[2].CausalParentEventId, Is.EqualTo(events[1].EventId));
            Assert.That(events[3].EventType, Is.EqualTo(CausalEventType.AgentAlerted));
        ...
        public void LostAgents_TraceBackThroughTheFlamesToABurningSquare()
        {
            ...
            var simulation = new Run(data);
            for (int i = 0; i < 60 * Run.TicksPerSecond; i++)
            {
                simulation.Step();
            }
            ...
            {
                if (record.EventType != CausalEventType.AgentLost)
                {
                    continue;
                }
                ...
                Assert.That(caught.EventType, Is.EqualTo(CausalEventType.AgentCaughtFire),
                ...
                while (cause.EventType == CausalEventType.AgentCaughtFire ||
                       cause.EventType == CausalEventType.ObjectCaughtFire)
                {
                    cause = simulation.EventLog.Get(cause.CausalParentEventId);
                }

                Assert.That(cause.EventType == CausalEventType.FireActivated || cause.EventType == CausalEventType.FireSpread,
        ...
        public void FireCrackle_TurnsSomeoneWithTheirBackToItUntilTheySeeIt()
        {
            ...
            for (int i = 0; i < 5 * Run.TicksPerSecond &&
        ...
        public void Yells_AlarmNearbyAndTurnHeadsFurtherAway()
        {
            ...
            foreach (CausalEvent record in simulation.EventLog.Events)
            {
                ...
                {
                    Assert.That(record.EventType, Is.EqualTo(CausalEventType.AgentNoticedSound));
                    Assert.That(record.CausalParentEventId, Is.EqualTo(yellId));
                }
                else if (record.SourceId == new SimulationId(3UL) && record.EventType == CausalEventType.AgentAlerted)
                {
                    Assert.That(record.CausalParentEventId, Is.EqualTo(yellId));
                }
            }
        ...
        public void YellAlarmedListener_TurnsTowardTheYellerWhileStartled()
        {
            ...
            {
                var run = new Run(data, seed);
                for (int i = 0; i < 40; i++)
                {
                    run.Step();
                    AgentSnapshot listener = run.GetAgent(1);
        ...
        public void Temperaments_RunFreezeForAWhileOrFreezeForGood()
        {
            ...
            int endTick = data.Fire.ActivationTick + 90 * Run.TicksPerSecond;
            while (simulation.Tick < endTick)
            {
                simulation.Step();
                for (int i = 0; i < count; i++)
        ...
        private static bool SomeoneIsDragging(Run simulation)
        {
            for (int i = 0; i < simulation.AgentCount; i++)
            {
        ...
        public void PanickedCrowds_BumpKnockDownTripAndGetBackUp()
        {
            ...
            for (ulong seed = 1UL; seed <= 8UL; seed++)
            {
                ...
                var simulation = new Run(data, seed);
                int count = simulation.AgentCount;
                var previous = new AgentSnapshot[count];
                ...
                // sprawled across another, which is a pile, not an overlap.
                long touching = data.World.OccupancyRadiusMillimetres * 2L - 50L;
                int endTick = data.Fire.ActivationTick + 30 * Run.TicksPerSecond;
                while (simulation.Tick < endTick)
                {
                    simulation.Step();
                    for (int i = 0; i < count; i++)
                    ...
                    switch (record.EventType)
                    {
                        case CausalEventType.AgentsCollided:
                            ...
                            Assert.That(log.Get(record.CausalParentEventId).EventType,
                                Is.EqualTo(CausalEventType.AgentScared).Or.EqualTo(CausalEventType.AgentCaughtFire));
                            break;
                        ...
                        case CausalEventType.AgentGotUp:
                            CausalEventType cause = log.Get(record.CausalParentEventId).EventType;
                            Assert.That(cause == CausalEventType.AgentKnockedDown ||
                                        cause == CausalEventType.AgentTripped ||
                                        cause == CausalEventType.AgentCrushed ||
                                        cause == CausalEventType.AgentCameTo ||

                                        // Somebody alight who threw themselves down
                                        // to roll gets up the same way.
                                        cause == CausalEventType.AgentRolled, Is.True,
