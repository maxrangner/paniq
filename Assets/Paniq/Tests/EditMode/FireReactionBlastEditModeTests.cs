...
namespace Paniq.Tests.EditMode
{
    ...
    /// </summary>
    public sealed class FireReactionBlastEditModeTests
    {
        ...
        private static void Blast(FireReactionSimulation simulation, LogicalPosition where)
        {
        ...
        public void TNT_MakesAHoleInTheWallThePlayerPointsAt()
        {
            ...
            FireReactionDoorSnapshot hole = simulation.GetDoor(simulation.DoorCount - 1);
        ...
        public void AHoleInAnOutsideWall_IsANewWayOut()
        {
            ...
            Assert.That(simulation.EventLog.Get(escaped[0].CausalParentEventId).EventType,
                Is.EqualTo(FireReactionEventType.PowerBlastedWall),
        ...
        public void NobodyEverTriesToOpenShutOrBatterAHole()
        {
            ...
            foreach (CausalEvent record in simulation.EventLog.Events)
            {
                ...
                Assert.That(record.EventType, Is.Not.EqualTo(FireReactionEventType.AgentTriedDoor));
                Assert.That(record.EventType, Is.Not.EqualTo(FireReactionEventType.AgentForcedDoor));
                Assert.That(record.EventType, Is.Not.EqualTo(FireReactionEventType.DoorClosed));
                Assert.That(record.EventType, Is.Not.EqualTo(FireReactionEventType.DoorLocked));
                Assert.That(record.EventType, Is.Not.EqualTo(FireReactionEventType.DoorBrokenDown));
            }
        ...
        public void TheBlast_ThrowsLooseThingsAwayFromItAndKnocksPeopleOver()
        {
            ...
            {
                new FireReactionPhysicsObjectDefinition(TheBox, PhysicsObjectKind.Box, new LogicalPosition(800, -4600), 400, 5000)
            };
            var simulation = new FireReactionSimulation(data);
        ...
        public void TheBang_FrightensPeopleWhoCouldNotHaveSeenAnything()
        {
            ...
            data.Agents = new[]
            {
                new FireReactionAgentDefinition(Somebody, new LogicalPosition(0, 4000), CardinalDirection.North,
                    AgentTraitValues.AllOrdinary)
            };
            var simulation = new FireReactionSimulation(data);
        ...
        public void TNT_IsRefusedWhereThereIsNoWallNearby()
        {
            ...
            Assert.That(EventsOfType(simulation, FireReactionEventType.PowerBlastedWall), Is.Empty);
        }
        ...
        public void TNT_IsRefusedWhereItWouldOverlapADoorThatIsAlreadyThere()
        {
            ...
            // The office's south door is centred on x = 2500 in the wall at z = -6000.
            Blast(simulation, new LogicalPosition(2500, -5900));
        ...
        public void TNT_RunsOutOfCharges()
        {
            ...
            Assert.That(EventsOfType(simulation, FireReactionEventType.PowerBlastedWall).Count, Is.EqualTo(charges),
        ...
        public void AHoleBetweenTwoRooms_LetsFireCross()
        {
            ...
            data.Rooms = new[]
            {
                new FireReactionRoomDefinition(west, new LogicalBounds(-6000, 0, -6000, 6000)),
                new FireReactionRoomDefinition(east, new LogicalBounds(0, 6000, -6000, 6000))
            };
            data.Doors = new[]
            {
                new FireReactionDoorDefinition(new SimulationId(2001UL), west, WallSide.East, 5000, 1000)
            };
            data.Agents = new[]
            {
                new FireReactionAgentDefinition(Somebody, new LogicalPosition(-5000, -5000), CardinalDirection.North,
                    AgentTraitValues.AllOrdinary)
            };
            ...
            Assert.That(EventsOfType(simulation, FireReactionEventType.PowerBlastedWall), Is.Not.Empty,
