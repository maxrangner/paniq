...
namespace Paniq.Tests.EditMode
{
    ...
    public sealed class HelpingEditModeTests
    {
        ...
        private ScenarioData EmptyRoom(params AgentDefinition[] people)
        {
            ScenarioData data =
        ...
        /// </summary>
        private Run HelperAndSomeoneFrozen(AgentTraitValues helper)
        {
            ScenarioData data = EmptyRoom(
                new AgentDefinition(Helper, new LogicalPosition(-2000, 0), CardinalDirection.North, helper),
                new AgentDefinition(InNeed, new LogicalPosition(0, 0), CardinalDirection.North,
        ...
        {
            Run simulation = HelperAndSomeoneFrozen(new AgentTraitValues(4, 4, 7, 9, 0, 4));
            Assert.That(simulation.GetAgent(1).Temperament, Is.EqualTo(AgentPanicTemperament.FreezeThenRun));
            for (int t = 0; t < 10 * Run.TicksPerSecond && EventsOfType(simulation, CausalEventType.AgentShookAwake).Count == 0; t++)
            {
                simulation.Step();
            }

            List<CausalEvent> shook = EventsOfType(simulation, CausalEventType.AgentShookAwake);
        ...
        {
            Run simulation = HelperAndSomeoneFrozen(new AgentTraitValues(4, 4, 7, 9, 9, 4));
            for (int t = 0; t < 10 * Run.TicksPerSecond; t++)
            {
                simulation.Step();
            }

            Assert.That(EventsOfType(simulation, CausalEventType.AgentShookAwake), Is.Empty);
        ...
        public void StrongKindRunner_DragsSomeoneKnockedOutThroughAnOpenDoor()
        {
            ...
            ScenarioData data = EmptyRoom(
                new AgentDefinition(Helper, new LogicalPosition(-2500, -1500), CardinalDirection.North,
                    new AgentTraitValues(8, 6, 8, 8, 1, 3)),
                new AgentDefinition(InNeed, new LogicalPosition(-2500, -3000), CardinalDirection.North,
            ...
            Assert.That(EventsOfType(simulation, CausalEventType.AgentPassedOut), Has.Count.EqualTo(1),
                "The box should have knocked them out.");
            List<CausalEvent> grabbed = EventsOfType(simulation, CausalEventType.AgentGrabbed);
            Assert.That(grabbed, Has.Count.EqualTo(1), "The helper never grabbed them.");
            Assert.That(grabbed[0].TargetId, Is.EqualTo(InNeed));
            List<CausalEvent> rescued = EventsOfType(simulation, CausalEventType.AgentRescued);
            ...
            Assert.That(simulation.EventLog.Get(rescued[0].CausalParentEventId).EventType,
                Is.EqualTo(CausalEventType.AgentEscaped), "Rescued because the helper got out.");
